...
function TestimonialCard({
  ...
  return (
    <div className="flex h-full w-[min(85vw,320px)] shrink-0 flex-col rounded-3xl border border-white/10 bg-black/50 p-6 md:w-80 md:p-8">
      ...
      <blockquote className="mt-4 flex-1 text-sm leading-relaxed text-white italic md:text-base">
        {item.comment}
      </blockquote>
      <div className="mt-6 border-t border-white/10 pt-4">
        <div className="flex items-center justify-between gap-3">
          <p className="font-semibold text-white">{item.name}</p>
          <span className="shrink-0 rounded-full bg-line-green/10 px-2 py-0.5 text-xs text-line-green">
            {item.role}
          </span>
        </div>
      </div>
